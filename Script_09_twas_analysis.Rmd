...
This script analyzes the results of the TWAS investigating the correlation between clock CpG DNAm and gene expression. 

This script does a few things:

1) Split the associations into cis (<100kB distance) and trans (>5MB distance or different chromosomes). Ranges between 100kB and 5MB were discarded.
2) For the trans-associations, select a core set of genes and CpGs and plot their t-statistics in a heatmap.
3) For both cis- and trans-associations, plot the number of associations per CpG in a bar chart.
4) Analyze overlap between each clock's associating cis- and trans-genes.
5) Analyze gene ontology (GO) enrichments for each clock's associating cis- and trans-genes.
...
#Plot the associations between the CpGs and the trans-genes in a heatmap.
...
#############
backup <- read.csv("test/sig.assoc.csv", row.names = 1)
sig.associations <- t.trans[rownames(backup),colnames(backup)]
dim(sig.associations)
#############
...
#Define three sidebars with genomic annotations for the CpGs.
