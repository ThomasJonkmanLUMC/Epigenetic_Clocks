...
This script was written to prepare genic feature regions for each gene. Gene-centric annotation was done based on the protein-coding genes in Ensembl build 75 (genome hg19).

Gene-centric annotation was split in 6 regions:
...
These features will be bound to the Infinium 450K CpGs in script 4, together with CGI-centric annotations. 

#CGI feature annotation

CGI-centric annotation was split in 3 regions:
1)	CGIs, as annotated by UCSC.
...
This following code was written to obtain chromatin state segments and histone modifications from Epigenomics Roadmap (accessed through the AnnotationHub package), and annotate these data to the Infinuim 450K CpGs. NB: the Epigenomics Roadmap contains chromatinstate data for 127 tissue types; for this analysis, we chose 7 primary blood cell types: PBMCs, monocytes, neutrophils, B-cells, CD4 T-cells, CD8-T-cells, and NK-cells. For histone modifications, only one cell type (PBMCs) was used; the reason for this is that in prelimianry analysis, chromatin states showed great overlap between cell types, so it doesn't really matter which cell type one chooses.
